...
int CConfig::LoadConf (const char *p_pcszFileName, int p_iCollectEmptyParam)
{
	...
	do {
		...
		while (! feof(psoFile)) {
			...
			/* »щем и затираем, символы перехода на новую строку */
			...
			/* «атираем разделитель и переводим указатель на значение параметра */
			*pcValue = '\0';
			++pcValue;
			/* ≈сли значение не задано переходим к следующей итерации */
